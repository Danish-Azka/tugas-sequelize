import Shop from "../models/shopModel.js";

export const createShop= async (req, res) => {
    try{
        const { nama, pass, email, divisi} = req.body;
        const shop = await Shop.create({ nama, pass, email, divisi});
        res.status(201).json(shop)
    } catch (error) {
        res.status(500).json({ error: error.message})
    }
}

export const getShop = async (req, res) => {
    try{
        const shop = await Shop.findAll()
        res.status(200).json(shop)
    }catch (error){
        res.status(500).json({ error : error.message})
    }
};

export const getShopById = async (req, res) => {
    try{
        const { id } = req.params;
        const shop = await Shop.findByPk(id);
        if (!shop) return res.status(404).json({message: 'ga ada'});
        res.status(200).json(shop)
    } catch (error){
        res.status(500).json({error : error.message})
    }
}

export const getShopByDivisi = async (req, res) => {
    try{
        const { divisi } = req.params;
        const shop = await Shop.findAll({where : {divisi : divisi}});
        if (!shop) return res.status(404).json({message: 'ga ada'});
        res.status(200).json(shop)
    } catch (error){
        res.status(500).json({error : error.message})
    }
}



export const updateShop = async (req, res) => {
    try {
        const { id } = req.params;
        const { nama, pass, email } = req.body;
        const shop = await Shop.findByPk(id);
        if (!shop) {
            return res.status(404).json({ message: 'Shop tidak ditemukan' });
        }
        await Shop.update({ nama, pass, email }, {
            where: { id }
        });
        const updatedShop = await Shop.findByPk(id);
        res.status(200).json(updatedShop);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};

export const deleteShop = async (req, res) => {
    try {
        const { id } = req.params;
        const shop = await Shop.findByPk(id);
        if (!shop) {
            return res.status(404).json({ message: 'Shop tidak ditemukan' });
        }
        await Shop.destroy({
            where: { id }
        });
        res.status(200).json({ message: 'Shop berhasil dihapus' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};

